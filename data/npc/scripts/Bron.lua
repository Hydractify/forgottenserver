...
local function greetCallback(cid)
	...
	if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 6 then
...
local function creatureSayCallback(cid, type, msg)
	...
	if isInArray({"recruitment", "violence", "outfit", "addon"}, msg) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) < 1 then
			npcHandler:say({
	...
	elseif(msgcontains(msg, "fist")) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 3 then
	...
	elseif(msgcontains(msg, "fighting spirit")) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 5 then
			if player:removeItem(5884, 1) then
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 6)
	...
	elseif(msgcontains(msg, "cloth")) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 7 then
	...
	elseif(msgcontains(msg, "silk")) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 8 then
			npcHandler:say("Oh, did you bring 10 rolls of spider silk yarn for me?", cid)
			npcHandler.topic[cid] = 9
		end
	elseif(msgcontains(msg, "sweat")) then
		if player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 9 then
	...
	elseif(msgcontains(msg, "yes")) then
		...
		elseif(npcHandler.topic[cid] == 2) then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 1)
		...
		elseif(npcHandler.topic[cid] == 4) then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 4)
			npcHandler.topic[cid] = 0
		elseif(player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 6 and npcHandler.topic[cid] == 0) then
			npcHandler:say({
		...
		elseif(npcHandler.topic[cid] == 7) then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 7)
			npcHandler.topic[cid] = 0
		elseif(npcHandler.topic[cid] == 8) then
			if player:getItemCount(5910) >= 50 and player:getItemCount(5911) >= 50 then
				...
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 8)
				npcHandler.topic[cid] = 0
			end
		elseif(npcHandler.topic[cid] == 9) then
			if player:removeItem(5886, 10) then
				...
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 9)
				npcHandler.topic[cid] = 0
			end
		elseif(npcHandler.topic[cid] == 10) then
			if player:removeItem(5885, 1) then
				...
				player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 10)
				npcHandler.topic[cid] = 0
			end
		elseif player:getStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon) == 10 then
			...
			player:setStorageValue(PlayerStorageKeys.OutfitQuest.BarbarianAddon, 11)
