...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'fugio') then
		if player:getStorageValue(PlayerStorageKeys.WhiteRavenMonasteryQuest.Diary) == -1 then
	...
	elseif msgcontains(msg, 'holy water') then
		local cStorage = player:getStorageValue(PlayerStorageKeys.TibiaTales.RestInHallowedGround.Questline)
		if cStorage == 1 then
	...
	elseif msgcontains(msg, 'amanda') and npcHandler.topic[cid] == 0 then
		if player:getStorageValue(PlayerStorageKeys.TibiaTales.RestInHallowedGround.Questline) == 1 then
	...
	elseif msgcontains(msg, 'yes') then
		if npcHandler.topic[cid] == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.WhiteRavenMonasteryQuest.Diary, 1)
		elseif npcHandler.topic[cid] == 2 then
			...
			player:setStorageValue(PlayerStorageKeys.WhiteRavenMonasteryQuest.Diary, 2)
		end
	elseif npcHandler.topic[cid] == 3 then
		...
		player:setStorageValue(PlayerStorageKeys.TibiaTales.RestInHallowedGround.Questline, 2)
