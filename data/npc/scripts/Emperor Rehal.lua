...
-- Justice For All
local missionKeyword = keywordHandler:addKeyword({'nokmir'}, StdModule.say, {npcHandler = npcHandler, text = 'Oh well, I liked Nokmir. He used to be a good dwarf until that day on which he stole the ring from Rerun.'}, function(player) return player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.JusticeForAll) == 1 end)
	missionKeyword:addChildKeyword({'grombur'}, StdModule.say, {npcHandler = npcHandler, text = 'He\'s very ambitious and always volunteers for the long shifts.'}, nil, function(player) player:setStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.JusticeForAll, 2) end)
...
local missionKeyword = keywordHandler:addKeyword({'mission'}, StdModule.say, {npcHandler = npcHandler, text = 'As you have proven yourself trustworthy I\'m going to assign you a special mission. Are you interested?'},
		function(player) return player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.RoyalRescue) < 1 and player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.JusticeForAll) == 5 end
	)
	...
	missionKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler,
		...
		}}, nil, function(player) player:setStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.RoyalRescue, 1) end
	)

local missionKeyword = keywordHandler:addKeyword({'mission'}, StdModule.say, {npcHandler = npcHandler, text = 'My son was captured by trolls? Doesn\'t sound like him, but if you say so. Now you want a reward, huh? ...'}, function(player) return player:getStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.RoyalRescue) == 5 end)
	missionKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler, text = 'Look at these dwarven legs. They were forged years ago by a dwarf who was rather tall for our kind. I want you to have them. Thank you for rescuing my son |PLAYERNAME|'},
		nil, function(player) player:setStorageValue(PlayerStorageKeys.hiddenCityOfBeregar.RoyalRescue, 6) player:addItem(2504, 1) end
	)
